...
createApp({
  ...
  computed: {
    computedList() {
      ...
      if (this.checked.select_secondType.length == 0) {
        ...
        newcardlist1 = this.filters(this.cardlist,this.checked.select_secondType);
      }
  ...
  methods: {
    new() {
      ...
      // URLSearchParams = 浏览器内置的API接口可以对URL进行处理 
    ...
    getdata() {
      ...
      let msort = urlParams.get("msort");
      let key = urlParams.get("key");
      // 宣告 city catagory 兩個新set (不可重複特性)
      const city = new Set();
      ...
      $.ajax({
        method: "POST",
        url: "./php/Product_jerry.php", //呼叫對象(php檔案位置)
        data: {
          // 要傳入php之資料
          msort: msort,
          key: key,
